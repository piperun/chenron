...
class ArchiveOrgClient {
  ...
  Future<String> archiveUrl(String targetUrl, {ArchiveOrgOptions? options}) async {
    try {
      ...
      if (response.statusCode == 200) {
        ...
        switch (data["status"]) {
          case "success":
            _logger.info('Archiving succeeded for $targetUrl. Archived URL: ${data['archived_snapshots']['closest']['url']}');
            return data["archived_snapshots"]["closest"]["url"];
          case "error":
            _logger.warning('Archiving failed for $targetUrl: \n message: ${data['message']} \n ${data['status_ext']}');
            throw Exception('Archiving failed: ${data['message']}');
          case "pending":
            _logger.info('Archiving in progress for $targetUrl. Job ID: ${data['job_id']}');
            return data["job_id"];
          default:
            ...
            _logger.severe('Unexpected status for $targetUrl: ${data['status']}');
